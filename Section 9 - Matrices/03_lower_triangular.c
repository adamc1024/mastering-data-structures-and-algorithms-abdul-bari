...
void rm_set(struct Matrix * M, int i, int j, int el) {
    if (i >= j) {
        M->A[row_major(i, j)] = el;
    }
}

int rm_get(struct Matrix M, int i, int j) {
    if (i >= j) {
        return M.A[row_major(i, j)];
    }
    else {
        return 0;
    }
...
void cm_set(struct Matrix * M, int i, int j, int el) {
    if (i >= j) {
        M->A[col_major(i, j, M->n)] = el;
    }
}

int cm_get(struct Matrix M, int i, int j) {
    if (i >= j) {
        return M.A[col_major(i, j, M.n)];
    }
    else {
        return 0;
    }
...
void cm_display(struct Matrix M) {
    ...
    for (i = 0; i < M.n; i++) {
        for (j = 0; j < M.n; j++) {
            if (i >= j) {
                printf("%d ", M.A[col_major(i, j, M.n)]);
            }
            else {
                printf("0 ");
            }
